...
reProg = re.compile("<BlastOutput_program>(?P<prog>[\w]*blast[\w]*)</BlastOutput_program>")
...
for record in blast:
   ...
   if record.alignments:
      ...
      for recHit in record.alignments:
         ...
         if "[" in recHit.title:
            recHitSp = (recHit.title).split("[")[1].split("]")[0].replace(",", ";")
         else:
         ...
         for hitHsp in recHit.hsps:
            ...
            if sProg == "blastx":
               hitHspQcov = (float(hitHsp.query_end - hitHsp.query_start + 1)/3)/queryLen
            else:
               # For blastn or blastp
               hitHspQcov = float(hitHsp.query_end - hitHsp.query_start + 1)/queryLen
            hitHspId = float(hitHsp.identities)/hitHspLen
            hitHspSim = float(hitHsp.positives)/hitHspLen

            #Printing to file
            lsMetrics = (query,queryLen,recHitAcc,recHitSp,recHitDsc,hitHspLen,hitHspEval,hitHspQcov,hitHspId,hitHspSim)
            print >> outFile, "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{:0.4f}\t{:0.4f}\t{:0.4f}".format(*lsMetrics)
